	...
	while (count < 100)
	{
		...
		for(int agent_id=0; agent_id<numAgents; agent_id++)
		{
...

			{
				...
				ICTS_agent_list.push_back(std::vector<int>{agent_id});
				ICTS_start_goal.push_back(std::make_pair(std::make_pair(init_x,init_y),
					std::make_pair(start_x,start_y)));
				int goal_time = std::abs(start_x-init_x)+std::abs(start_y-init_y);
				ICTS_task_times.push_back(std::make_pair(std::make_pair(0,goal_time),0));

				ICTS_agent_list.push_back(std::vector<int>{agent_id});
				ICTS_start_goal.push_back(std::make_pair(std::make_pair(start_x,start_y),
					std::make_pair(goal_x,goal_y)));
				int start_time = goal_time;
				goal_time += std::abs(goal_x-start_x)+std::abs(goal_y-start_y);
				ICTS_task_times.push_back(std::make_pair(std::make_pair(start_time,goal_time),0));
				ICTS_task_edges.push_back(std::make_pair(ICTS_task_id,ICTS_task_id+1));
				ICTS_current_task[agent_id] = std::make_pair(ICTS_task_id+1,goal_time);
				ICTS_task_id+=2;
			}

			{
				agent_inits.push_back(std::make_pair(init_x,init_y));
				CBS_agent_list.push_back(std::vector<int>{agent_id});
				CBS_start_goal.push_back(std::make_pair(std::make_pair(start_x,start_y),
					std::make_pair(goal_x,goal_y)));
				int start_time = std::abs(start_x-init_x)+std::abs(start_y-init_y);
		...
		for(int task_id=numAgents; task_id<numTasks; task_id++)
		{
			int assign_num;
			do
			{
...

			{
				...
				for(int i=0; i<agents_assigned_list.size(); i++)
				{
					...
					int start_time = ICTS_current_task[agents_assigned_list[i]].second;
			...
			}
			{
				...
				for(int i=0; i<agents_assigned_list.size(); i++)
				{
					...
					int init_x = CBS_start_goal[carry_task_pre].second.first;
					int init_y = CBS_start_goal[carry_task_pre].second.second;
					int go_end_time = CBS_current_task[agents_assigned_list[i]].second +
						std::abs(start_x-init_x)+std::abs(start_y-init_y);
