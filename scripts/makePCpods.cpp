...
int main(int argc, char *argv[])
{
	...
	int graph_no = 0;
	Graph graph;
	create_vertices(graph,get(&VProp::state,graph),graph_file_name +  "graph" + std::to_string(graph_no) 
		+ ".graphml",2,get(&EProp::prior,graph));
	...
	while (count < 100)
	{
		// std::cout << "Start making the problem\n";
		// std::cin.get();
		count++;
		...
		boost::unordered_map <Vertex, int> assigned_vertices;

		std::vector <Vertex> q1, q2, q3, q4;
		for(auto v: vertex_list){
			std::pair <int,int> cord = findCoordinate(graph,v);
			if(cord.first <= 5){
				if(cord.second <= 5){
					q1.push_back(v);
				}
		...
		std::vector<int> taskStarts (numTasks*numAgents);
		std::vector<int> taskGoals (numTasks*numAgents);
		...
		for(int i=0; i<numTasks; i++){
			...
			for(int j=0; j<numAgents; j++){
				task_nums.push_back(j);
			}
			random_shuffle(task_nums.begin(), task_nums.end());
			for(int j=0; j<numAgents; j++){
				int task_no = i*numAgents+j;
				std::cout << "Task No = " << task_no << std::endl;
				if(i==0){
					edgeMap[task_no] = -1;
					for(auto v:vertex_list){
						if(assigned_vertices.find(v) == assigned_vertices.end()){
							assigned_vertices[v] = 1;
							taskStarts[task_no] = v;
							break;
						}
					}
					for(auto v:vertex_list){
					...
					int pred_task = (i-1)*numAgents+task_nums[j];
					edgeMap[task_no] = pred_task;
				...
				std::cout << "Pred Task No = " << edgeMap[task_no] << std::endl;
			}
		}
		...
		std::vector <std::pair <Vertex,Vertex>> ICTS_tasks (numTasks*numAgents);
		std::vector <std::vector<int>> ICTS_task_agents (numTasks*numAgents);
		std::map <int, int> ICTSLastTask;
		// greedily assign agents to tasks
		while(true){
			//find not assigned tasks
			std::vector <int> not_assigned_tasks;
			for (int i=0; i<numTasks*numAgents; i++){
			...
			for (auto task: not_assigned_tasks){
				int pred_task = edgeMap[task];
				if(pred_task !=-1 && tasksToAgents.find(pred_task) == tasksToAgents.end())
					continue;
		...
		file_stream << std::to_string(ICTS_task_edges.size()+edgeMap.size()-numAgents) + "\n\n";
