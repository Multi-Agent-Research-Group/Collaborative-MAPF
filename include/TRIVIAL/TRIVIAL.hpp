...
public:
	...
	TRIVIAL(cv::Mat img, std::string& leftRoadmapFileName, std::string& rightRoadmapFileName,
	...
	std::pair<Graph, std::pair<Vertex,Vertex> > 
...
TRIVIAL::TRIVIAL(cv::Mat img, std::string& leftRoadmapFileName, std::string& rightRoadmapFileName,
		Eigen::VectorXd _start_config, Eigen::VectorXd _goal_config, bool insert_start_goal, bool use_dijkstra)
	: mImage(img)
	...
	, mRightRoadmapFileName(rightRoadmapFileName)
{
	...
	std::pair<Graph, std::pair<Vertex,Vertex> > return_value
		 = constructEpsilonGraph(input_left_graph,input_left_start_vertex,input_left_goal_vertex);
		
	left_graph = return_value.first;
	Vertex left_start_vertex = return_value.second.first;
	Vertex left_goal_vertex = return_value.second.second;
	...
	Vertex right_start_vertex = return_value.second.first;
	Vertex right_goal_vertex = return_value.second.second;
...
void TRIVIAL::displayPath(std::vector<Eigen::VectorXd> path)
{
	cv::Mat image;
...
std::pair<Graph, std::pair<Vertex,Vertex> > 
	TRIVIAL::constructEpsilonGraph(Graph input_graph, Vertex start_vertex, Vertex goal_vertex)
{
	// double epsilon_value = 0.01;
	Graph new_graph;
	boost::unordered_map<Vertex,Vertex> sourceMap;
	...
	for(boost::tie(ei,ei_end) = edges(input_graph); ei!=ei_end;++ei)
	{
		...
		// add vertices 
		Vertex previous_vertex = sourceMap[u];
		for (unsigned int i = 1; i < nStates-1; i++)
		{
			...
			std::pair<Edge,bool> curEdge = boost::add_edge(previous_vertex, new_node, new_graph);
			new_graph[curEdge.first].prior = 1.0;
			new_graph[curEdge.first].length = epsilon_value;
			previous_vertex = new_node;
		}
		{
			std::pair<Edge,bool> curEdge = boost::add_edge(previous_vertex, sourceMap[v], new_graph);
			new_graph[curEdge.first].prior = 1.0;
			new_graph[curEdge.first].length = epsilon_value;
		}
	}
...
void TRIVIAL::preprocess_graph(Graph &g, Vertex & _goal, bool use_dijkstra)
{
	if(use_dijkstra)
	{
...
std::vector<CompositeVertex> TRIVIAL::getNeighbors(CompositeVertex &v)
{
...
std::vector<CompositeVertex> TRIVIAL::AStar()
{
...
// ===========================================================================================
std::vector<Eigen::VectorXd> TRIVIAL::solve()
{
...
bool TRIVIAL::evaluateConfig(Eigen::VectorXd config)
{
...
// ===========================================================================================
bool TRIVIAL::evaluateEdge(const CompositeEdge& e) // returns false if in collision
{
...
// ===========================================================================================
CompositeEdge TRIVIAL::getEdge(CompositeVertex u, CompositeVertex v) const
{
	CompositeEdge uv;
	bool edgeExists;
...
// ===========================================================================================
double TRIVIAL::estimateCostToCome(CompositeVertex v) const
{
	// return graph[v].distance + graph[v].lazyCostToCome;
	return graph[v].distance;
}

double TRIVIAL::heuristicFunction(CompositeVertex v) const
{
	return graph[v].heuristic;
}

double TRIVIAL::estimateTotalCost(CompositeVertex v) const
{
	return estimateCostToCome(v) + heuristicFunction(v);
}

double TRIVIAL::getDistance(Eigen::VectorXd config_one, Eigen::VectorXd config_two) 
{
	// double epsilon_value = 0.01;
