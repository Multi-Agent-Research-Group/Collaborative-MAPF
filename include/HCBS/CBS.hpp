...
public:
	...
	CBS(int numAgents, int numRobots, std::vector<std::string> roadmapFileNames, 
		std::vector<Eigen::VectorXd> startConfig, std::vector<Eigen::VectorXd> goalConfig, 
		std::vector<int> startTimesteps, std::vector<int> goalTimesteps, 
		...
		int &search_num)
	{
		...
		ISPS planner(mNumAgents,mRoadmapFileNames,mStartConfig,mGoalConfig,
			mStartVertex, mGoalVertex, constraints, mMap);
		...
		while(PQ.PQsize()!=0)
		{
			...
			if(!checkCoupling(p.shortestPaths, agent_id_1, constraint_1, agent_id_2, constraint_2))
			{
				...
				ISPS planner1(mNumAgents,mRoadmapFileNames,mStartConfig,mGoalConfig, 
					mStartVertex, mGoalVertex, increase_constraints_agent_id_1, mMap);
			...
			ISPS planner2(mNumAgents,mRoadmapFileNames,mStartConfig,mGoalConfig,
				mStartVertex, mGoalVertex, increase_constraints_agent_id_2, mMap);
