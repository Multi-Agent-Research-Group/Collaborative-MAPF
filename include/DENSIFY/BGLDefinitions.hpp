...
///////////////////////// DEFINITIONS FOR INDIVIDUAL ROADMAP
  ...
  ///////////////////////// DEFINITIONS FOR DIRECTED EPSILON ROADMAP

  // Properties associated with each roadmap vertex.
  struct DensifiedVProp
  {
    /// The underlying state of the vertex
    //State (configuration)
    Eigen::VectorXd state;

    /// vertex is also a part of the original roadmap
  ...
  // Helpful alias declarations
  /// Undirected Boost graph
  typedef boost::adjacency_list<boost::vecS, boost::vecS,  boost::bidirectionalS, DensifiedVProp, DensifiedEProp> DensifiedGraph;

  /// Boost vertex
  typedef boost::graph_traits<DensifiedGraph>::vertex_descriptor DensifiedVertex;

  /// Boost vertex iterator
  typedef boost::graph_traits<DensifiedGraph>::vertex_iterator DensifiedVertexIter;

  /// Boost edge
  typedef boost::graph_traits<DensifiedGraph>::edge_descriptor DensifiedEdge;

  /// Boost edge iterator
  typedef boost::graph_traits<DensifiedGraph>::edge_iterator DensifiedEdgeIter;

  /// Boost graph out edge iterator
  typedef boost::graph_traits<DensifiedGraph>::out_edge_iterator DensifiedOutEdgeIter;

  /// Boost graph in edge iterator
  typedef boost::graph_traits<DensifiedGraph>::in_edge_iterator DensifiedInEdgeIter;

  /// Boost graph neighbor iterator
  typedef boost::graph_traits<DensifiedGraph>::adjacency_iterator DensifiedNeighborIter;

  /// Map each vertex to a unique ID
  typedef boost::property_map<DensifiedGraph, size_t DensifiedVProp::*>::type DensifiedVPIndexMap;

  /// Map each vertex to the underlying state [read from the graphml file]
  typedef boost::property_map<DensifiedGraph, Eigen::VectorXd DensifiedVProp::*>::type DensifiedVPStateMap;

  /// Map each edge to a unique ID
  typedef boost::property_map<DensifiedGraph, boost::edge_index_t DensifiedEProp::*>::type DensifiedEdgeIndexMap;

  /// Map each edge to its length
  typedef boost::property_map<DensifiedGraph, double DensifiedEProp::*>::type DensifiedEPLengthMap;

  /// Map each edge to its existence prior
  typedef boost::property_map<DensifiedGraph, double DensifiedEProp::*>::type DensifiedEPPriorMap; 
  ...
  {
    DensifiedVertex left_vertex;

    DensifiedVertex right_vertex;
