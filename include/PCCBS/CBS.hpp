...
public:
	...
	void printStats()
	{
		...
		std::cout<<"Collision Hash time: "<<mCHTime.count()/1000000.0<<std::endl;
				...
				if(target_task_ids[i]==-1 || target_task_ids[i]!=target_task_ids[j])
				{
					...
					if(getVerticesCollisionStatus(mGraphs[agent_ids[i]][target_vertices[i]].state, mGraphs[agent_ids[j]][target_vertices[j]].state))
					{
						...
						if(target_in_delivery[j] == false)
						{
							...
							safe_j = (mTasksList[agent_ids[j]][target_tasks_completed[j]].second.first == target_vertices[j] || mTasksList[agent_ids[j]][target_tasks_completed[j]].second.second == target_vertices[j]);
		...
		while(PQ.PQsize()!=0)
		{
			...
			// if(numSearches%2 == 0)
			{
				...
				for(int agent_id=0; agent_id<mNumAgents; agent_id++)
				{
					...
					PRINT<<"Cost: "<<p.costs[agent_id]<<std::endl;
		...
		int current_makespan, std::vector<std::vector<SearchState> > &shortestPaths, std::vector<int> &consider_agents)
	{
		...
		while(pq.PQsize()!=0)
		{
			...
			// std::cout<<"CV: "<<current_vertex<<" CT: "<<current_timestep<<" CTC: "<<current_tasks_completed
			// 	<<" CID: "<<current_in_delivery<<std::endl;
